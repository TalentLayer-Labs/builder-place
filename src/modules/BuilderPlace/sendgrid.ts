...
// export const sendNormalMessage = async (to: string, subject: string, html: string) => {
...
export const sendTransactionalEmailValidation = async (to: string) => {
  if (!APIKEY || !SENDERS_EMAIL) {
    console.warn('No APIKEY or SENDERS_EMAIL, skipping email.');
    return;
  }
  ...
  //TODO integrate template from wbe3mail
